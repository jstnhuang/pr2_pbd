...
#include "pbd_landmarks/SaveLandmark.h"
...
CaptureRoi::CaptureRoi()
    : nh_(),
      ...
      save_client_(
          nh_.serviceClient<pbd_landmarks::SaveLandmark>("save_landmark")) {}
...
bool CaptureRoi::ServeCapture(rapid_msgs::CaptureRoi3DRequest& req,
    ...
    pbd_landmarks::SaveLandmarkRequest save_req;
    save_req.landmark.name = "Untitled landmark";
    save_req.landmark.cloud = output;
    pbd_landmarks::SaveLandmarkResponse save_res;
    save_client_.call(save_req, save_res);
    res.id = save_res.id;
    ROS_INFO("Landmark ID: %s", save_res.id.c_str());

    return true;
}

int main(int argc, char** argv) {
    ros::init(argc, argv, "capture_landmark");
    ros::AsyncSpinner spinner(4);
    spinner.start();
    ros::NodeHandle nh;
    CaptureRoi capture_server;
    ros::Subscriber pc_sub = nh.subscribe(
        "cloud_in", 1, &CaptureRoi::CloudCallback, &capture_server);
